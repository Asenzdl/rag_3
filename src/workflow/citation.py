...
4. **异常降级**：CitationExtractionError 继承 NonRetryableError，
   重试无意义（LLM 重输出产生的引用格式可能仍然不符预期），
   调用方应捕获后返回空列表而非中断主流程。
...
class CitationExtractionError(NonRetryableError):
    """引用提取失败时抛出。

    为什么同时继承 NonRetryableError（设计决策）：
        引用提取失败通常因为 LLM 输出格式不符合预期，重试 LLM 会产生
        不同的输出（非幂等），且额外 LLM 调用不值得。继承 NonRetryableError
        确保上游重试机制不会对此异常重试。
    ...
    为什么不应中断主流程（反直觉辩护）：
        引用提取是增强功能，回答文本本身仍然有效。调用方应捕获此异常，
        返回空引用列表，而非让整个请求失败。
    """
    pass
