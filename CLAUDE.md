...
每次 Task 执行过程中，**必须在以下时机自动更新** `project_info/CONTEXT_INDEX.md`，无需用户提醒：
- **阶段1 设计完成时**：若本 Task 新增/删除/重命名了模块文件或公共类，更新「核心模块定位表」
- **阶段2 代码实现完成时**：更新「核心模块定位表」中的主类/函数和职责；更新「关键配置变量表」；更新 `__init__.py` 导出信息
- **Task 状态变更时**：更新「📍 当前 Task」和「Task 进度」表中对应 Task 的状态和产出
